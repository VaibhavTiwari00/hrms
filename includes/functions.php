...
function getRandomMixedCharacter()
{
	$type = mt_rand(1, 3); // Randomly choose between 1, 2, or 3 for small letters, capital letters, or numbers respectively

	switch ($type) {
		case 1:
			$randomChar = chr(mt_rand(97, 122)); // ASCII values for small letters a-z
			break;
		case 2:
			$randomChar = chr(mt_rand(65, 90)); // ASCII values for capital letters A-Z
			break;
		case 3:
			$randomChar = chr(mt_rand(48, 57)); // ASCII values for numbers 0-9
			break;
		default:
			$randomChar = ''; // Default to empty string if type is not matched
	}

	return $randomChar;
}

function generateMixedValue($minLength)
{
	$length = max($minLength, 10); // Set a minimum length of 10 characters
	$mixedValue = '';

	while (strlen($mixedValue) < $length) {
		$mixedValue .= getRandomMixedCharacter();
	}

	return substr($mixedValue, 0, $length); // Trim to the required length
}
...
if (isset($_POST['add_user'])) {
	...
	$random_user_id = generateMixedValue(15);
